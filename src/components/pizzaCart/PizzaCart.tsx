...
export const PizzaCart: React.FC<CartTypes> = ({
  title,
  types,
  sizes,
  price,
  ...
  const onClickPlus = () =>
    dispatch<{ payload: string; type: string }>(
      cartSlice.actions.plusOrder(identity)
    );
  const onClickMinus = () =>
    dispatch<{ payload: string; type: string }>(
      cartSlice.actions.minusOrder(identity)
    );
  const onClickRemove = () =>
    dispatch<{ payload: string; type: string }>(
      cartSlice.actions.removeItem(identity)
    );
    ...
    <div className={s.cart__item}>
      ...
      <div className={s.cart__item_count}>
        <button
          ...
          className={`${sb.button} ${sb.button__outline} ${sb.button__circle}`}
        >
